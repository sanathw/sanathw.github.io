...
void setupUI()
{
  with (pjsCM)
  {
    ...
    setupCM(SHOW_AS_HORIZONTAL, SHOW_AT_BOTTOM, 6, START_OPENED);
    ...
    bEnter = addButton(.75, .15, .15, .5, "Enter");
    bRestart = addButton(.75, .75, .15, .15, "restart");
...
void processUI()
{
  if (bEnter != null && bEnter.doProcess == true) 
  {
    delay = 0;
    currentHand++;
    if (currentHand > 3) currentHand = 0;
  }
  
  if (bRestart != null && bRestart.doProcess == true) 
  {
    delay = 0;
    setupCards(true);
    currentHand = 0;
