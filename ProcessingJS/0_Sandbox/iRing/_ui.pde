...
void setupUI()
{
  with (pjsCM)
  {
    ...
    //setContainer(c1);
    //b1 = addButton(.012, .1, .12, .31, "Save Data");
...
void processUI()
{
  //if (b1 != null && b1.doProcess == true) {saveData();}
}
...
void resetData()
{
  ...
  //"6"
  if (currentLine >= totalLines) return; line = lines[currentLine]; currentLine++; 
  
  //number of points
  if (currentLine >= totalLines) return; line = lines[currentLine]; currentLine++; 
  int points = parseInt(line);
   ...
   for (int i = 0; i < points; i++)
   {
     ...
     inputStroke.add(new PVector(x, y));
   }
}
