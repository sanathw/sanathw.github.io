...
void calculateProbability()
{
  ...
  for (int xl = -h_sw; xl < (massPos.x-massRadius); xl++)
  {
  ...
  for (int xr = (massPos.x+massRadius)+1; xr < h_sw; xr++)
  {
...
float CalProbB(int xi, int yi, int lazerXi, int lazerYi, ArrayList holes)//, int ti, boolean useTime)
{
  ...
  for (int h = 0; h<holes.size(); h++)
  {
    ...
    float distDiff = (r - shortestDist) * 10.0;
    float popLoss = 1 / (1 + (distDiff * distDiff));
    
    //r = r / 2.0;
    float d = r*r/200; // controls the number of interference lines // 50 bigger less
...
void draw()
{
  ...
  int lastX = -h_sw;
  int lastY = (int) (screenY - (screenPropbaility[lastX] * 1000));
  for (int x= lastX+1; x<h_sw; x++)
  {
    int y = (int) (screenY - (screenPropbaility[x] * 1000));
